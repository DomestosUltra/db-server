...
INSERT INTO leaders (last_name, birth_date) VALUES
    ('Иванов', '1980-01-01'),    -- будет обслуживать только автобусные туры
    ('Петров', '1995-06-15'),    -- смешанные туры
    ('Сидоров', '1999-12-30'),   -- только автобусные туры
    ('Кузнецов', '1975-03-20'),  -- железнодорожные и авиа
    ('Попов', '1990-09-10'),     -- только автобусные туры
    ('Васильев', '1985-11-25'),  -- смешанные туры
    ('Михайлов', '1998-07-07'),  -- только авиа туры
    ('Новиков', '1970-05-05'),   -- железнодорожные туры
    ('Федоров', '1992-02-14'),   -- автобусные туры
    ('Морозов', '1988-08-08');   -- смешанные туры

-- Заполнение таблицы туров (разные типы, категории, страны и ценовые диапазоны)
INSERT INTO tours (tour_type, price, start_date, city_id, tour_category) VALUES
    -- Автобусные туры (для демонстрации 1-го запроса)
    ...
    -- Железнодорожные туры (для демонстрации 2-го запроса)
    ...
    -- Авиатуры в Турцию и Египет (для демонстрации 3-го запроса)
    ...
    -- Туры-шопинги (для расчета средней стоимости в 3-м запросе)
    ...
    -- Дополнительные туры для разнообразия
    ('авиа', 60000, '2025-11-01', 11, 'деловой'),
    ('железнодорожный', 35000, '2025-10-20', 1, 'обычный'),
    ('автобусный', 14000, '2025-11-15', 2, 'экскурсионный');

-- Заполнение таблицы заказов с разными датами для временных условий
INSERT INTO orders (order_date, order_number, tour_id, leader_id, participants) VALUES
    -- Заказы за последние 3 месяца (июль-сентябрь 2025)
    -- Автобусные туры для руководителей, обслуживающих только автобусные
    ('2025-09-01', 'ORD001', 1, 1, 15),   -- Иванов - автобусный, высокая стоимость
    ('2025-09-05', 'ORD002', 2, 1, 20),   -- Иванов - автобусный, высокая стоимость
    ('2025-08-15', 'ORD003', 3, 3, 12),   -- Сидоров - автобусный
    ('2025-09-10', 'ORD004', 4, 3, 25),   -- Сидоров - автобусный, очень высокая стоимость
    ('2025-08-20', 'ORD005', 5, 5, 18),   -- Попов - автобусный
    ('2025-09-12', 'ORD006', 1, 9, 16),   -- Федоров - автобусный
    
    -- Смешанные туры для других руководителей (исключают их из 1-го запроса)
    ('2025-09-02', 'ORD007', 6, 2, 20),   -- Петров - железнодорожный
    ('2025-08-25', 'ORD008', 1, 2, 15),   -- Петров - автобусный
    ...
    ('2025-09-01', 'ORD019', 16, 6, 12),  -- шопинг тур
    ('2025-09-08', 'ORD020', 17, 2, 15),  -- шопинг тур
    ('2025-09-14', 'ORD021', 18, 10, 10); -- шопинг тур
